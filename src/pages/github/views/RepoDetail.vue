	...
	export default {
		...
		methods   : {
			...
			getRepoDetail(username, reponame) {
				return api(`https://api.github.com/repos/${username}/${reponame}`).then(repo => {
					this.repo = repo;
				});
			},
			...
			getRepoContents(username, reponame) {
				return api(`https://api.github.com/repos/${username}/${reponame}/contents`).then(contents => {
					this.contents = contents.sort((a, b) => a.type.localeCompare(b.type));
				});
			},
			getRepoContribs(username, reponame) {
				return api(`https://api.github.com/repos/${username}/${reponame}/contributors`).then(contribs => {
					this.contribs = contribs;
				});
			},
			getRepoLanguages(username, reponame) {
				return api(`https://api.github.com/repos/${username}/${reponame}/languages`).then(languages => {
					const newLanguages = Object.keys(languages).map(key => ({name: key, value: languages[key]}));
					let total = 0;
			...
			scrollEvent() {
				const {top,bottom} = this.$refs.tabwrapper.parentElement.getBoundingClientRect();
				// console.log('top',top);
