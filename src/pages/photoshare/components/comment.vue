			...
			<ul class = "c-list clearfix">
				<li v-for = "(v,k) in replyData">
					<div class = "c-top clearfix">
						...
						<span v-if = "id==v.commentFromMemberId" class = "icon iconfont icon-lajixiang delete" @click = "del(v.commentId)"></span>
						<a @click = "doReply" :commentId = "v.commentFromMemberId" :name = "v.fromMemberNickname" class = "reply">
							<span class = "iconfont icon icon-more2" :commentId = "v.commentFromMemberId" :name = "v.fromMemberNickname"></span>
							回复
						</a>
					</div>
					<div class = "c-bottom">
