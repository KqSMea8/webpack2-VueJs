	...
	<div class = "container story">
		...
		<div class = "row" v-else>
			...
			<div class = "story-actions">
				<a class = "btn-floating btn-large waves-effect waves-light purple lighten-2" @click = "removeFavoriteStory" v-if = "favorite">
					<i class = "material-icons">favorite</i>
				</a>
				<a class = "btn-floating btn-large waves-effect waves-light purple lighten-2" @click = "addFavoriteStory" v-else = "favorite">
					<i class = "material-icons">favorite_border</i>
				</a>
				<!-- 返回上级按钮 -->
				<a class = "btn-floating btn-large waves-effect waves-light purple lighten-2">
					<i class = "material-icons" @click = "goBack">keyboard_return</i>
				</a>
			</div>
		</div>
	</div>
...
<script>
	...
	export default {
		...
		methods : {
			// 获取文章
			getStory(storyId) {
				this.$http.get(`${this.$url}/api/4/news/${storyId}`).then(res => {
					if (res.data.images == undefined) {
						res.data.images = ['http://pic1.zhimg.com/56d1d1202077c7b5b0e48e3b7d3ebb60_t.jpg']
					}
					this.story = res.data
					this.isFav()
			...
			getLongComments(storyId) {
				this.$http.get(`${this.$url}/api/4/story/${storyId}/long-comments`).then(res => {
			...
			getShortComments(storyId) {
				this.$http.get(`${this.$url}/api/4/story/${storyId}/short-comments`).then(res => {
			...
			isFav(){
				...
				let test = array.find((n) => n == this.story.id)
				// console.log(array, test, this.story.id);
		...
		computed: {
		  ...
		  // 过滤后的body正文
			storyBody() {
				return this.story.body.replace(/headline/, '').replace(/img\-place\-holder/, '').replace(/view\-more/, '').replace(/查看知乎讨论/, '').replace(/https?\:\/\//g, 'https://images.weserv.nl/?url=')
			}
		}
	}
</script>
