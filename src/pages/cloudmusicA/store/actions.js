...
export default {
	...
	getComments: ({commit, state, dispatch}) => {
		commit(types.CHANGE_LOADING_STATE)
		return API.getCommments(state.player.currentTrackInfo.commentThreadId).then(({data: {hotComments: comments}}) => {
			// comments为一个对象数组
	 ...
	 * 获取歌曲URL地址
	 */
	getSongs   : ({commit, state, dispatch}, {newtrack}) => {
		API.getSongs(state.tracks[newtrack].commentThreadId).then(({data: {data}}) => {
			// console.log(data)
			// console.log(data[0].url)
			commit(types.MP3_URL, data[0].url)
			let params = {
				urlType: 0,
				url    : data[0].url
			}
			commit(types.UPDATE_URL, params)
		}).catch((e) => {
			dispatch('showPopup', {
				msg      : 'Error:' + e.message,
				autodes  : 2500,
