<template>
	<div>
		<carousel>
			<!--以下组件CarouselItem被放置在carousel的slot中 -->
			<carousel-item v-for = "(item, index) in top_stories" :key = "item.id">
		...
		<div class = "list" v-for = "day in list">
			...
			<router-link :to = "`detail/${item.id}`" v-for = "item in day.stories" class = "media" :key = "item.id">
	...
	export default {
		...
		methods   : {
			loadMore () {
				...
				this.isLoading = true
				store.dispatch('FETCH_BEFORE').then(() => {
	    ...
	    // todo-vue:运用scrollBehavior记录页面离开前位置示例
