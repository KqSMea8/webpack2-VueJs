		...
		<div class = "comment-wrap" ref = "comment">
			<div class = "music-info">
				...
				<div class = "music-name">
					<p v-if = "JSON.stringify(curPlayMusic) !== '{}'">{{curPlayMusic.detail.name}}</p>
					<p>
						<span v-for = "(item,index) in curPlayMusic.detail.ar">
							{{item.name}}
			...
			<div class = "recent-comments" v-show = "!showLoading">
				...
				<ul class = "comment-content">
					<li v-for = "(item,index) in comments">
						...
						<div class = "comment-body">
							<div class = "comment-body-title">
								<p>{{item.user.nickname}} <br /><span>2017年7月7日</span>
								</p>
								<p>{{item.likedCount}} <i class = "iconfont icon-dianzan"></i>
								</p>
							</div>
							<div class = "comment-body-content">
								<p>{{item.content}}</p>
