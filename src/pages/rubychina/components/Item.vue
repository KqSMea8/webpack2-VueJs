	...
	<div class = 'topic'>
		...
		<div class = 'topic-avatar'>
			<router-link :to = "'/users/' + item.user.id">
				<img :alt = "item.user.login" :src = "item.user.avatar_url" style = "width: 48px; height: 48px; border-radius: 24px;" />
			</router-link>
		</div>
		<div class = 'topic-infos'>
			...
			<div class = 'topic-title'>
				<router-link :to = "'/topics/' + item.id">
					<span class = "topic-node">{{item.node_name}}</span> {{item.title}}
			...
			<div class = 'topic-info'>
				...
				<span v-else> · 最后由
					<router-link :to = "'/users/' + item.last_reply_user_id">{{item.last_reply_user_login}}</router-link>
				              回复于 {{ replyTime }}
				</span>
			</div>
		</div>
